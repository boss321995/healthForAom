...
- หากใช้บริการฐานข้อมูลที่ไม่ให้สิทธิ์รัน SQL เอง ให้ส่งไฟล์ `database/postgres_schema.sql` ให้ผู้ดูแลระบบนำไปสร้างตารางทั้งหมด
- หากมีสิทธิ์เข้าถึง PostgreSQL ด้วยตัวเอง ให้ทำตามขั้นตอน:
  ...
  ```bash
  psql -h <HOST> -U <USER> -d health_management -f database/postgres_schema.sql
  ```

- ไฟล์ `server/health_management.sql` และ `database/health_management.sql` ยังคงเก็บไว้เป็นสคีมาสำหรับ MySQL/อ้างอิงเท่านั้น ไม่ต้องนำไปใช้กับ PostgreSQL
...
- **PostgreSQL Schema** - ใช้ไฟล์ `database/postgres_schema.sql` สำหรับสร้างตารางบน production DB
